...
import openAISalesService from '../services/openai-sales.service';
import offtoService from '../services/offto.service';
import sessionService from '../services/session.service';
import { ChatCompletionMessageParam } from 'openai/resources/chat/completions';

export class ChatSalesController {
  /**
  ...
  async sendMessage(req: Request, res: Response) {
    try {
      ...
      // Get session context
      const context = sessionService.getContext(phone) as any;
      ...
      let selectedBasketId: string | null = null;
      let selectionMethod = '';
      ...
      if (basketIdMatch && (context.flightResults || context.hotelResults)) {
        ...
        const flightMatch = context.flightResults?.find((f: any) => f.basket_id?.toString() === basketId);
        const hotelMatch = context.hotelResults?.find((h: any) => h.basket_id?.toString() === basketId);
      ...
      if (selectedBasketId) {
        ...
        if (context.flightResults) {
          const flight = context.flightResults?.find((f: any) =>
            f.basket_id?.toString() === selectedBasketId
          );
        ...
        if (context.hotelResults) {
          const hotel = context.hotelResults?.find((h: any) =>
            h.basket_id?.toString() === selectedBasketId
          );
      ...
      const result = await openAISalesService.processMessage(messages, context);
      ...
      if (result.flightBooking) {
        ...
        const isInvalidBasketId = !params.basketId ||
                                  params.basketId === 'selected_flight_basket_id' ||
                                  params.basketId === 'YOUR_BASKET_ID' ||
                                  params.basketId.toString().toUpperCase().includes('BASKET') ||
                                  isNaN(Number(params.basketId));

        if (isInvalidBasketId) {
          if (context.selectedBasketId) {
            ...
            // No valid basketId available
            console.error('❌ No valid basketId available for booking');
            res.json({
              success: false,
              message: 'عذراً، لم أتمكن من العثور على معرف الحجز. الرجاء اختيار رحلة أولاً.\nSorry, I could not find the booking ID. Please select a flight first.'
            });
            return;
          }
        }
        ...
        try {
          const bookingResult = await offtoService.bookFlight(
            params.basketId,
            {
...
Please click the payment link above to complete your booking. Thank you for choosing us! 🙏`;
            ...
            sessionService.addMessage(phone, 'user', message);
            sessionService.addMessage(phone, 'assistant', successMessage);
...
Just tell me what you're looking for (flights or hotels) with your travel details.`;
              ...
              sessionService.addMessage(phone, 'user', message);
              sessionService.addMessage(phone, 'assistant', expiredMessage);
              ...
              // Get conversation history
              const messagesWithError: ChatCompletionMessageParam[] = sessionService.getMessages(phone).map(msg => ({
                role: msg.role as 'user' | 'assistant',
                content: msg.content
              }));
              ...
              const aiErrorResponse = await openAISalesService.processMessage(messagesWithError);
              ...
              sessionService.addMessage(phone, 'assistant', errorResponseText);
      ...
      if (result.hotelBooking) {
        ...
        try {
          const bookingResult = await offtoService.bookHotel(
            params.basketId,
            {
              title: params.title,
...
Just tell me what you're looking for (flights or hotels) with your travel details.`;
              ...
              sessionService.addMessage(phone, 'user', message);
              sessionService.addMessage(phone, 'system' as any, errorNote);

              // Get conversation history including the error note
              const messagesWithError: ChatCompletionMessageParam[] = sessionService.getMessages(phone)
      ...
      if (result.flightSearch) {
        ...
        try {
          const searchResult = await offtoService.getBasketFlights(
            params.arrivalCode,
            params.outboundDate,
            params.returnDate || null,  // Pass null for one-way flights
          ...
          if (searchResult.success && searchResult.baskets && searchResult.baskets.length > 0) {
            const flightsDisplay = searchResult.baskets.slice(0, 5).map((basket: any, i: number) =>
      ...
      if (result.hotelSearch) {
        ...
        try {
          const searchResult = await offtoService.searchHotels(
          ...
          if (searchResult.success && searchResult.hotels && searchResult.hotels.length > 0) {
            const hotelsDisplay = searchResult.hotels.slice(0, 5).map((hotel: any, i: number) => {
      ...
      // Save AI response
      sessionService.addMessage(phone, 'assistant', finalResponse);
...
export default new ChatSalesController();
